...
applyFilter (JqField fieldName n) obj@(Object _) = join (noteNotFoundError fieldName (applyFilter n <$> obj ^? key fieldName))
applyFilter (JqIndex index n) array@(Array _) = join (noteOutOfRangeError index (applyFilter n <$> array ^? nth index))
